var checkbox = new mdc.checkbox.MDCCheckbox(document.querySelector('.mdc-checkbox'));
var formField = new mdc.formField.MDCFormField(document.querySelector('.login-form'));
...
if (pass != null) {
    ...
    crypt_file.onreadystatechange = function () {
        if (crypt_file.readyState == XMLHttpRequest.DONE) {
            try {
                var dec = code.decryptMessage(crypt_file.responseText, Base64.decode(pass));
                //console.log(dec);
                eval(dec);
                document.getElementsByClassName("login-container")[0].remove();
